...
import aiomysql
try:
    from dingtalk_gpt_bot.services.synchronize import Settings, OpenApiBase
except Exception:
    from synchronize_data import Settings, OpenApiBase
...
async def main():
    ...
    target_name = "80631001-SBV视频-核心词-0.65-广泛-7.27"
    ...
    if is_sb:
        ...
        sku_summary = _calc_derived(sku_sums, start_14, end_14)
        primary_zero = _is_all_zero_summary(sku_summary)
        if primary_zero and campaign_type_lower.startswith("sp"):
    ...
    if (is_sb or campaign_type_lower.startswith("sp")) and primary_zero and fallback_zero:
