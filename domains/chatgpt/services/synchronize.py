...
def _aes_encrypt(key: str, data: str) -> str:
    try:
        ...
        try:
            from Cryptodome.Cipher import AES  # pycryptodomex fallback
        except Exception as exc:
            raise RuntimeError(
                "Missing AES backend. Install pycryptodome or pycryptodomex."
            ) from exc
...
class ParamInvalidError(ValueError):
    """OpenAPI 参数不合法（code=102）。用于触发上层降级重试。"""
...
class Settings:
    ...
    run_queryword_reports: bool = False
    # ---------------------------------
    run_sp_keyword_reports: bool = False
    run_sp_target_hour_reports: bool = False
    run_sb_target_reports: bool = False
    # ---------------------------------
    run_names: bool = False
    run_ad_groups: bool = False
    run_sb_creativity: bool = False
    run_sp_product_ads: bool = False
    # ---------------------------------
    run_sb_targeting_dim: bool = False
    # ---------------------------------
    date_list_override: List[str] = field(default_factory=list)
    # 默认使用手动指定的起止日期；可按需修改
    report_start_date: str = "2025-12-14"
    report_end_date: str = "2025-12-17"
...
async def _fetch_with_pagination(op_api: OpenApiBase, access_token: str, route: str, base_payload: dict,
                                 ...
                                 raise_on_param_invalid: bool = False,
    ...
    async def _fetch_page(offset_val: int) -> Tuple[int, Optional[ResponseResult], Optional[Exception]]:
        ...
        for attempt in range(1, retries + 1):
            ...
            if resp.code not in (0, 200):
                if resp.code == 102 and raise_on_param_invalid:
                    return offset_val, None, ParamInvalidError(f"code={resp.code} msg={resp.message}")
                if attempt == retries:
    ...
    while True:
        ...
        if exc is not None:
            if isinstance(exc, ParamInvalidError) and raise_on_param_invalid:
                raise exc
...
async def get_query_word_data_for_seller(op_api: OpenApiBase, access_token: str,
    ...
    # 对 code=102 做降级重试：先用当前 target_type，失败后试 ALL，再试不带 target_type。
    payload_candidates: List[Tuple[dict, str]] = []
    normalized_target_type = (target_type or "").strip()
    if normalized_target_type:
        payload_candidates.append(
            ({"sid": sid, "report_date": report_date, "target_type": normalized_target_type}, normalized_target_type)
        )
    if normalized_target_type.upper() != "ALL":
        payload_candidates.append(
            ({"sid": sid, "report_date": report_date, "target_type": "ALL"}, "ALL")
        )
    payload_candidates.append(
        ({"sid": sid, "report_date": report_date}, "ALL")
    )

    for idx, (base_payload, effective_target_type) in enumerate(payload_candidates):
        try:
            ...
            if idx == len(payload_candidates) - 1:
                print(f"[error] route {route} sid={sid} name={seller_name} 参数降级后仍失败: {exc}")
                return []
            print(f"[warn] route {route} sid={sid} name={seller_name} target_type={effective_target_type} 参数不合法，尝试降级...")
...
async def fetch_campaign_names(op_api: OpenApiBase, access_token: str, seller: dict,
    ...
    async def _fetch(route: str, tag: str) -> List[list]:
        ...
        payload_candidates = [
            {"sid": sid},
            {"sid": sid, "sponsored_type": "ALL"},
            {"sid": sid, "sponsored_type": tag.upper()},
        ]
        for idx, payload in enumerate(payload_candidates):
            try:
                ...
                if idx == len(payload_candidates) - 1:
                    print(f"[error] route {route} sid={sid} name={seller_name} 参数降级后仍失败: {exc}")
                    return []
                print(f"[warn] route {route} sid={sid} name={seller_name} 参数不合法，尝试降级 payload: {payload}")
        return []
...
async def fetch_ad_groups(op_api: OpenApiBase, access_token: str, seller: dict,
    ...
    async def _fetch(route: str, tag: str) -> List[list]:
        ...
        payload_candidates = [
            {"sid": sid, "state": "enabled"},
            {"sid": sid},
            {"sid": sid, "sponsored_type": "ALL"},
            {"sid": sid, "state": "enabled", "sponsored_type": "ALL"},
        ]
        for idx, payload in enumerate(payload_candidates):
            try:
                return await _fetch_with_pagination(
...
async def fetch_sb_creativity(op_api: OpenApiBase, access_token: str, seller: dict,
                              sem: asyncio.Semaphore, page_size: int, retries: int) -> List[list]:
    """查询 SB 创意（/pb/openapi/newad/hsaProductAds），返回 [sid, campaign_id, ad_group_id, ad_creative_id, asin] 列表，仅 state=enabled。"""
    sid = seller.get("sid")
    if not sid:
        return []
    ...
    def _process(item: dict, rows: List[list]):
        ...
        if ad_creative_id is None or ad_creative_id == "":
            # 无 ad_creative_id 跳过，表定义要求该字段非空并为主键一部分
            return
        if ad_group_id is None or ad_group_id == "":
            # 无 ad_group_id 跳过，表定义要求该字段非空并为主键一部分
            return
        ...
        if isinstance(asin_field, list):
            for a in asin_field:
                if a is None or (isinstance(a, str) and not a):
                    continue
                rows.append([sid, campaign_id, str(ad_group_id), str(ad_creative_id), str(a)])
        # 如果 asin 是字符串，直接使用
        elif isinstance(asin_field, str) and asin_field:
            rows.append([sid, campaign_id, str(ad_group_id), str(ad_creative_id), asin_field])
        # 否则不添加行（避免插入空 asin）
...
async def _run_sb_creativity(op_api: OpenApiBase, access_token: str, target_sellers: List[dict],
    ...
    for r in all_rows:
        ...
        if ad_creative_val is None or ad_creative_val == "" or not asin_val or not ad_group_val:
            continue
