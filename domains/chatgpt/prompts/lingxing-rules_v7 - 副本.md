...
| 项 | 说明 |
| --- | --- |
| 必填字段 | `campaign_name`、`sku`、`compare_7d.current/prev`、`compare_14d.current/prev`（各含 `date_range` 与全部指标） |
| 选填字段 | `best_week`/`recommended_settings`（仅持续变差）；`sku_14d_all`、`thresholds`、`negative_rules_target`、`negative_rules_ad`、`negative_rules_ad_groups`（始终返回，可为空） |
